...
function paintPreviousStep(i) {
  ...
  blocks.forEach((item, index) => {
    if (
      (item.className.includes("pre-line") ||
        item.className.includes("post-line")) &&
...
function paintCurrentStep(i) {
  ...
  blocks.forEach((item, index) => {
    if (
      ...
      item.classList.remove("point-done");
      item.classList.add("point-active");
    } else if (item.className.includes("content-item")) {
      item.classList.add("content-item__active");
      item.classList.remove("content-item__disabled");
    }
  });
}
